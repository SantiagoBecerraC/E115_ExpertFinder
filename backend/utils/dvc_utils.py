...
class DVCManager:
    ...
    def _run_command(self, command, description):
        ...
        try:
            result = subprocess.run(command, cwd=str(self.project_root), capture_output=True, text=True, check=True)
    ...
    def setup_remote(self, remote_url=None):
        ...
        result = subprocess.run(["dvc", "remote", "list"], cwd=str(self.project_root), capture_output=True, text=True)
        ...
        return self._run_command(
            ["dvc", "remote", "add", "-d", "storage", remote_url], f"add remote storage at {remote_url}"
        )
        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                if line.strip():
                    parts = line.split("|", 2)
                    if len(parts) == 3:
                        history.append({"commit_hash": parts[0], "date": parts[1], "message": parts[2]})
