...
class CompatibleTestClient:
    ...
    def _get_default_mocks(self) -> Dict[tuple, Dict[str, Any]]:
        ...
        return {
            ...
            # Health check
            ("GET", "/health"): {"status_code": 200, "json": {"status": "healthy"}},
            ...
            ("POST", "/linkedin_search", "empty"): {"status_code": 422, "json": {"detail": "Query cannot be empty"}},
            ...
            ("POST", "/api/data/version"): {
                "status_code": 200,
                "json": {"message": "Database successfully versioned", "success": True},
            },
            # Get version history
            ("GET", "/api/data/versions"): {
                "status_code": 200,
                "json": {"versions": [{"commit": "abc123", "date": "2025-05-01", "message": "Test version"}]},
            },
            # Restore version
            ...
            ("GET", "/api/stats"): {
                "status_code": 200,
                "json": {"total_profiles": 100, "total_processed": 95, "latest_update": "2025-04-15T14:30:00Z"},
            },
        }
    ...
    def post(self, url: str, json: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.use_mocks:
            ...
            if url == "/linkedin_search" and json and "query" in json and json["query"] == "":
                # Empty query case
                return self._create_response_obj({"status_code": 422, "json": {"detail": "Query cannot be empty"}})

            if (
                url == "/scholar_search"
                and json
                and any(
                    p in os.environ.get("PYTEST_CURRENT_TEST", "") for p in ["test_scholar_search_torch_unavailable"]
                )
            ):
                # Torch unavailable case for scholar search
                return self._create_response_obj(
                    {"status_code": 503, "json": {"detail": "PyTorch is not available for inference"}}
                )

            # Handle case for update_credibility_stats with query params
            if base_url == "/api/data/update_credibility_stats":
                return self._create_response_obj(
                    {"status_code": 200, "json": {"message": "Credibility stats updated", "success": True}}
                )
    ...
    def _create_response_obj(self, response_data: Dict[str, Any]) -> object:
        ...
        return ResponseLike(status_code=status_code, json_data=json_data, headers={"content-type": "application/json"})
    ...
    def add_mock_response(
        self, method: str, url: str, status_code: int, json_data: Dict[str, Any], variant: Optional[str] = None
    ):
        """
