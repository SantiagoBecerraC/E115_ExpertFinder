...
class TestScholarAgent(unittest.TestCase):
    ...
    def test_retrieve_articles(self, mock_print, mock_chat_openai, mock_state_graph):
        ...
        def mock_invoke(query, n_results=None):
            if query == "machine learning":  # Phase 1: Direct search
                return [
                    {
                        'metadata': {'doc_type': 'author', 'author': 'Prof A'},
    ...
    def test_retrieve_articles_exception(self, mock_print, mock_chat_openai, mock_state_graph):
        ...
        self.assertTrue(any("Error retrieving articles" in str(call) for call in mock_print_calls), 
    ...
    def test_rerank_articles(self, mock_print, mock_chat_openai, mock_state_graph):
        ...
        mock_reranker.rerank.return_value = [
            {
                'document': {'text': 'Article about machine learning by Professor A'},
                'score': 0.9
            },
            {
                'document': {'text': 'Article about deep learning by Professor B'},
                'score': 0.5
            }
        ]
        ...
        formatted_results = [
            {
                'author_profile': {
                    'metadata': {'author': 'Professor A'}
                },
                'website_content': [
                    {'text': 'Article about machine learning by Professor A'}
                ],
                'journal_content': [
                    {'text': 'Journal article by Professor A'}
                ]
            }
        ]
    ...
    def test_rerank_articles_exception(self, mock_print, mock_chat_openai, mock_state_graph):
        ...
        self.assertTrue(any(("Error" in call and "API error" in call) for call in mock_print_calls), 
