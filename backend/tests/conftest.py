...
def fixture_profiles_dir():
    ...
    return os.path.join(
        Path(__file__).parent, "fixtures", "test_data", "profiles"
    )
...
def fixture_processed_profiles_dir():
    ...
    return os.path.join(
        Path(__file__).parent, "fixtures", "test_data", "processed_profiles"
    )
...
def fastapi_test_client():
    ...
    if use_real_server:
        ...
        if is_service_reachable(base_url.replace("http://", "").split(":")[0], 
...
def api_client():
    ...
    # Root endpoint
    client.add_mock_response(
        "GET", "/", 200, {"message": "Expert Finder API is running"}
    )
    ...
    client.add_mock_response(
        "POST", "/search", 400, 
        {"detail": "max_results must be a positive number"},
        "negative"
    )
    
    # Version endpoints
    client.add_mock_response(
        "POST", "/api/data/version", 200, 
        {"message": "Database successfully versioned"}
    )
    
    client.add_mock_response(
        "GET", "/api/data/version/history", 200, 
        {
            "versions": [
                {"commit_hash": "abc123", "date": "2025-04-01", "message": "Test version"}
            ]
        }
    )
    
    client.add_mock_response(
        "POST", "/api/data/version/restore", 200, 
        {"message": "Version restored successfully"}
    )
    
    # Add mock responses for Google Scholar endpoints
    client.add_mock_response(
        "POST", "/scholar_search", 200,
        {
            "experts": [
                {
                    "id": "scholar_1",
