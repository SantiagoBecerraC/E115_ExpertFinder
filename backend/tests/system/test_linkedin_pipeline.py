...
class TestLinkedInPipeline:
    ...
    def sample_profile(self, test_data_path):
        ...
        if "experience" in profile_data and isinstance(profile_data["experience"], list) and len(profile_data["experience"]) > 3:
        ...
        if "education" in profile_data and isinstance(profile_data["education"], list) and len(profile_data["education"]) > 2:
    ...
    def test_vectorize_and_add_to_chroma(self, sample_profile, chroma_db_path, test_collection_name, temp_output_dir):
        ...
        with patch('chromadb.PersistentClient', return_value=client):
            ...
            with patch.object(vectorizer, 'download_profiles_from_gcp', return_value=True), \
                 patch.object(vectorizer, 'get_profiles_in_collection', return_value=set()):
    ...
    def test_full_linkedin_pipeline(self, sample_profile, chroma_db_path, test_collection_name, temp_output_dir):
        ...
        try:
            ...
            assert "credibility_score" in profile_data or "credibility_factors" in profile_data, "Credibility information missing from profile"
        except Exception as e:
