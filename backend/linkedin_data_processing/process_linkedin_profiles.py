...
def download_unprocessed_profiles_from_gcp(storage_client, local_dir="/tmp/profiles"):
    ...
    try:
        ...
        profile_blobs = [
            blob for blob in blobs if blob.name.endswith(".json") and not blob.name.endswith("errors.json")
        ]
...
def extract_profile_data(file_path):
    ...
    try:
        ...
        # Profile flags
        extracted_data.update({"student": profile.get("student", False)})
        ...
        if languages:
            ...
            for lang in languages:
                lang_data = {"name": lang.get("name", ""), "proficiency": lang.get("proficiency", "")}
        ...
        if "educations" in extracted_data:
            has_phd = any(
                "ph" in edu.get("degree", "").lower() or "doctor" in edu.get("degree", "").lower()
                for edu in extracted_data["educations"]
            )
            has_masters = any(
                "master" in edu.get("degree", "").lower()
                or "ms" == edu.get("degree", "").lower()
                or "mba" in edu.get("degree", "").lower()
                for edu in extracted_data["educations"]
            )
            has_bachelors = any(
                "bachelor" in edu.get("degree", "").lower()
                or "bs" == edu.get("degree", "").lower()
                or "ba" == edu.get("degree", "").lower()
                for edu in extracted_data["educations"]
            )
        ...
        if "experiences" in extracted_data:
            ...
            is_executive = any(
                ...
                for term in ["ceo", "cto", "cfo", "coo", "chief", "president", "founder", "owner", "partner"]
            )
...
def process_profiles_and_upload_to_gcp(
    temp_dir="/tmp/profiles", gcp_folder="linkedin_data_processing/processed_profiles"
):
...
def download_new_processed_profiles_for_rag(
    storage_client,
    existing_profiles,
...
def setup_chroma_db(persist_directory="chroma_db"):
    ...
    try:
        ...
        collection = chroma_client.create_collection(
            name="linkedin_profiles", metadata={"hnsw:space": "cosine"}  # Use cosine similarity
        )
...
def search_profiles_demo(query, filters=None, top_k=5, chroma_dir="chroma_db"):
    ...
    results = collection.query(
        query_embeddings=[query_embedding.tolist()], n_results=top_k, where=where_clause if where_clause else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action", choices=["process", "prepare_rag", "search", "all"], default="all", help="Action to perform"
    )
    parser.add_argument("--chroma_dir", default="chroma_db", help="Directory to persist ChromaDB data")
    parser.add_argument(
        "--query",
        default="machine learning experts with experience at top tech companies",
    ...
    parser.add_argument("--top_k", type=int, default=5, help="Number of results to return (for search action)")
    parser.add_argument(
        "--force", action="store_true", help="Force processing of all profiles, even if already processed"
    )
