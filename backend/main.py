...
app = FastAPI(title="Expert Finder API", description="API for finding experts from various sources", version="1.0.0")
...
async def search_scholar_experts(search_query: SearchQuery):
    ...
    try:
        ...
        return {"experts": scholar_experts, "total": len(scholar_experts), "source": "scholar"}
...
async def search_linkedin_experts(search_query: SearchQuery):
    try:
        ...
        expert_json_data = linkedin_agent.find_experts_json(
            search_query.query, initial_k=search_query.max_results * 2, final_k=search_query.max_results
        )
        ...
        return {"experts": linkedin_experts, "total": len(linkedin_experts), "source": "linkedin"}
