...
def process_scholar_data(json_file):
    ...
    authors_with_articles = defaultdict(lambda: {
        'author_info': {},
        'articles': []
    })
    ...
    if not articles:
        ...
        print(f"Available keys in data: {list(data.keys()) if isinstance(data, dict) else 'Data is a list'}")
        return {}
...
def prepare_chroma_data(authors_data):
    ...
    return {
        'authors': authors_collection_data,
        'articles': articles_collection_data
    }
